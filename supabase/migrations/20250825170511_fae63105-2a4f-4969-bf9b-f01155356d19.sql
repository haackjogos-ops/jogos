-- Fix ambiguous column reference in advance_fila_with_offline_check function
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.fila f WHERE f.status = 'ativo') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.fila f WHERE f.status = 'ativo') THEN
      ...
      SET status = 'ativo', iniciou_em = now()
      WHERE f.id = (
        SELECT id FROM public.fila f2 WHERE f2.status = 'pendente' ORDER BY f2.ordem LIMIT 1
      );
    END IF;
  END IF;
